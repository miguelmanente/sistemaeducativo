...
class ModuloListados:
    ...
    def __init__(self, root, db_path="bdescuela.db"):
        self.root = root
        self.db_path = db_path
        self.root.title("Exportación de Listados de Docentes")
    ...
    def _crear_interfaz(self):
        frame = ttk.LabelFrame(
            self.root, text=" Configuración del Reporte ", padding=15
        )
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.cerrar_pantalla)
    ...
    def generar_pdf(self):
        ...
        if not registros:
            messagebox.showwarning(
                "Sin datos",
                f"No se encontraron docentes con situación: {situacion}",
            )
            return
